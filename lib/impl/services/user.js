...
module.exports = function (options) {
  ...
  function registerUser (args, done) {
    ...
    that.make$('client').load$({appkey: appkey, fields$: ['name']}, function (err, client) {
      if (err) {
        return done(null, {ok: false, why: err})
      }
      ...
      that.act('role: concorda, cmd: createClient', {appkey: appkey, name: appkey}, function(err, client){
        if (err) {
          return done(null, {ok: false, why: err})
        }
  ...
  function setUserClients (msg, response) {
    var userId = msg.userId
