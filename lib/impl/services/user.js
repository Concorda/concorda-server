...
module.exports = function (options) {
  ...
  function createUser (msg, response) {
    ...
    context.act('role: auth, validate: password', userData, function(err){
      if (err) {
        return response(null, {ok: false, why: err})
      }
  ...
  function registerUser (args, done) {
    ...
    if (!appkey){
      return done(null, {ok: false, why: 'No appkey provided.'})
    }

    that.act('role: concorda, cmd: loadSettings', function (err, settings){
      if (err) {
        return done(null, {ok: false, why: err})
      }
      settings = settings.data

      var registerType = settings.publicRegister
      if (appkey === 'concorda'){
        registerType = settings.concordaPublicRegister
      }

      var token = args.token
      delete args.token

      if (registerType && registerType !== '1'){
        // verify token
        if (!token){
          return done(null, {ok: false, why: 'Register available only by invitation.'})
        }
      ...
      function verifyToken(token, email, cb){
        that.make$(clientToken).load$({token: token, email: email}, function (err, tokenData) {
          if (err) {
            return done(err)
          }
