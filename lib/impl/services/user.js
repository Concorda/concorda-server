...
module.exports = function (options) {
  ...
  function createUser (msg, response) {
    ...
    userData.validateEmail = _.contains(userData, 'validateEmail') ? userData.validateEmail : true
    userData.forcePwdChange = _.contains(userData, 'forcePwdChange') ? userData.forcePwdChange : true

    context.act('role: auth, validate: password', userData, function(err, response){
      if (err) {
  ...
  function updateUser (msg, response) {
    ...
    if (userData.password || userData.repeat){
      context.act('role: auth, validate: password', userData, function(err, result) {
        if (err) {
          return response(null, {ok: false, why: 'Internal server error'})
        }
        if (!result.ok) {
