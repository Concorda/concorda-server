...
module.exports = function (options) {
  ...
  function changePassword (msg, response) {
    ...
    context.make$('sys', 'user').load$({id: user.id}, function (err, user) {
      ...
      context.act('role: auth, validate: password', user, function(err){
        if (err){
          return response(null, {ok: false, why: err})
        }
  ...
  function validatePassword(user, done){
    ...
    context.act('role: concorda, cmd: loadSettings', function (err, settings){
      ...
      if (!data.password) {
        return done('Password Required')
      }
      if (!data.repeat) {
        return done('Repeat password required')
      }
      if (data.password !== data.repeat) {
        return done('Confirm Password must match Password')
      }
      if (data.password === data.email) {
        return done('Cannot set password equal with email')
      }

      if(data.password.length < settings.minLength) {
        return done(`Error: Password must contain at least ${settings.minLength} characters!`)
      }
      ...
      if (settings.requireNumeric) {
        ...
        if (!re.test(data.password)) {
          return done("Error: password must contain at least one number (0-9)!")
        }
      }
      ...
      if (settings.requireLowercase) {
        re = /[a-z]/;
        if (!re.test(data.password)) {
          return done("Error: password must contain at least one lowercase letter (a-z)!")
        }
      }
      ...
      if (settings.requireUppercase) {
        re = /[A-Z]/;
        if (!re.test(data.password)) {
          return done("Error: password must contain at least one uppercase letter (A-Z)!")
        }
      }
      return done()
