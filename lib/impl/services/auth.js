    ...
    context.act('role: concorda, cmd: loadClient', {appkey: appkey}, function (err, result) {
      ...
      context.act('role: concorda, cmd: loadSettings', function (err, settingsResp) {
        ...
        context.prior(msg, function (err, out) {
          ...
          // everything is ok, login was executed, now I should verify if I must delete some of the older sessions
          verifyActiveSessionNumber(settings, user, function () {
            response(err, out)
          })
        })
      })

      function verifyActiveSessionNumber (settings, user, cb) {
        if (!settings || !settings.userPolicy || !settings.userPolicy.maximumSessionNumber) {
          return cb()
        }
        let limit = parseInt(settings.userPolicy.maximumSessionNumber, 10)

        if (limit === 0) {
          return cb()
        }

        // need to use native because postgres store is not accepting
        // 'when' column as this is a reserved word
        context.make$().native$(function (err, client) {
          if (err || !client) {
            return cb('Cannot get client.')
          }

          client.query(`SELECT * FROM sys_login WHERE "user"='${user.id}' ORDER BY "when" DESC`, function (err, res) {
            if (err) {
              return cb()
            }
            ...
            // now keep only the 'limit' number of active session, all other will be deleted
            let noOfActiveSessions = 0
            let toRemove = []
            res.rows.forEach(function (row) {
              if (!row.active) {
                toRemove.push(row.id)
              }
              else if (noOfActiveSessions >= limit) {
                toRemove.push(row.id)
              }
              else {
                noOfActiveSessions++
            ...
            Async.each(toRemove, function (id, callback) {
              context.make$('sys', 'login').remove$({id: id}, callback)
            }, cb)
          })
        })
      }
