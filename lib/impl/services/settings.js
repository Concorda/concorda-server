...
module.exports = function (options) {
  ...
  function loadSettings (msg, response) {
    ...
    context.make$(settingsEntity).load$({}, function (err, settings) {
      if (err) {
  ...
  function saveSettings (msg, response) {
    ...
    context.make$(settingsEntity).load$({}, function (err, dbsettings) {
      if (err) {
        return response(null, {ok: false, why: err})
      }
  ...
  // this action was supposed to be called on first Concorda run.
  ...
  function getDefaultValues () {
    ...
    // by default public register is active for other application's users
    settings.publicRegister = (process.env.PUBLIC_REGISTER || '1')

    // by default public register is not active for Concorda users
    settings.concordaPublicRegister = (process.env.CONCORDA_PUBLIC_REGISTER || '0')
