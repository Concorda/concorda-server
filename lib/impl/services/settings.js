...
module.exports = function (options) {
  ...
  function loadSettings (msg, response) {
    ...
    this.make$(settingsEntity).load$({}, function (err, settings) {
      ...
      if (settings && settings.id){
        ...
        return response(null, {ok: true, data: prepareSettingsForUI(settings)})
      }

      // settings not in DB - strange but I must make sure there are some default settings - adding them now
      this.make$(settingsEntity, {configured: false, register_type: 'public'}).save$(function (err, settings){
        if (err) {
          return response(null, {ok: false, why: err})
        }
  ...
  function prepareSettingsForDB (settings) {
    let auth = {}
    if (settings.authType) {
      if (_.isArray(settings.authType)) {
        for (let i in settings.authType) {
          auth[settings.authType[i]] = true
        }
      }
      else {
        auth[settings.authType] = true
      }
      settings.authType = auth
    }

    return settings
  }

  function prepareSettingsForUI (settings) {
    let auth = []
